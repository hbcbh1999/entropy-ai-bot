...
module.exports = class BotInstance extends EventEmitter {
  ...
  onEntropyResponse(to_recipient, message) {
    this.emit('entropy-message', to_recipient, message);
  }

  forwardWireMessage(message, from, cb) {
    //call login endpoint
    try{
      this.service.sendToEntropyMessage(this.botID, message, from,  cb);
    }catch(err){
