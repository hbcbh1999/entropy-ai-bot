...
const http = require('http'); // can avoid using https as long as the request is rerouted locally
...
module.exports = class BotInstance extends EventEmitter {
  ...
  onEntropyResponse(from, message) {
    this.emit('entropy-message', from, message);
  }

  onWireMessage(message, cb) {
    //call login endpoint
    //TODO: some unique user identifier - not just a conversation with pre-existing user
    console.log('WARNING: using static uid and session_sid - need to rewrite');
    var post_data = querystring.stringify({
      'uid': '192.168.1.2',
      'text': message
    });
    this.service.sendToEntropyMessage(post_data, this.botID, cb);
  }
