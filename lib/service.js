...
class Service {
  ...
  entropyResponse(req, data, res, botID) {
    const self = this;
    console.log(`New request for bot ${botID}`);
  ...
  conversationMessageAdd(req, data, res, botID) {
    ...
    if (!Service.validateConversationMessageAdd(data.data)) {
      ...
      console.log(`New message for bot ${botID}`);
  ...
  sendToEntropyMessage(post_data, botID, cb){
    post_data = _.assign(
      {
        // variables required by wire to link the conversation back ot the user
        'conversation': this.bots[botID].data.conversation.id,
        'time':this.bots[botID].data.time,
        'botID': botID,
        'from': this.bots[botID].lastMessageData.data.from,
        'sender': this.bots[botID].lastMessageData.data.sender,
        'recipient': this.bots[botID].lastMessageData.data.recipient
      },
      post_data
    );
